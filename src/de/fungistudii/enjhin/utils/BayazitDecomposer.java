...
import static de.fungistudii.enjhin.utils.GeometryUtils.areVerticesClockwise;
import static de.fungistudii.enjhin.utils.GeometryUtils.toFloatArray;

/**
 * Taken from <a
 * href="http://code.google.com/p/box2d-editor/source/browse/editor/src/aurelienribon/bodyeditor/maths/earclipping/bayazit/BayazitDecomposer.java">Aurelien
 * Ribon's Physics Body Editor</a><br>
 * <br>
 * Convex decomposition algorithm created by <a
 * href="http://mnbayazit.com/">Mark Bayazit</a>
 * <a href="http://mnbayazit.com/406/bayazit">more information</a> about this
 * algorithm
 */
public abstract class BayazitDecomposer {
    ...
    /**
     * Decompose the polygon into several smaller non-concave polygon. If the
     * polygon is already convex, it will return the original polygon, unless it
     * is over Settings.MaxPolygonVertices. Precondition: Counter Clockwise
     * polygon
     */
    ...
    /**
     * This method detects if two line segments (or lines) intersect, and, if
     * so, the point of intersection. Use the firstIsSegment and secondIsSegment
     * parameters to set whether the intersection point must be on the first and
     * second line segments. Setting these both to true means you are doing a
     * line-segment to line-segment intersection. Setting one of them to true
     * means you are doing a line to line-segment intersection test, and so on.
     * Note: If two line segments are coincident, then no intersection is
     * detected (there are actually infinite intersection points).
     *
     ...
     * @param firstIsSegment Set this to true to require that the intersection
     * point be on the first line segment.
     * @param secondIsSegment Set this to true to require that the intersection
     * point be on the second line segment.
     * @param point This is set to the intersection point if an intersection is
     * detected.
    ...
    public static boolean lineIntersect(Vector2 point1, Vector2 point2, Vector2 point3, Vector2 point4, boolean firstIsSegment, boolean secondIsSegment, Vector2 point) {
        ...
        if (!(denom >= -EPSILON && denom <= EPSILON)) {
            ...
            if (!firstIsSegment || ua >= 0.0f && ua <= 1.0f) {
                ...
                // segment 1 as well.
                if (!secondIsSegment || ub >= 0.0f && ub <= 1.0f) // check if they are coincident (no collision in this case)
                {
                    if (ua != 0f || ub != 0f) {
    ...
    public static class SimplifyTools {
         ...
         * general recursive version that does not use the speed-up technique by
         * using the Melkman convex hull. If you pass in 0, it will remove all
         * collinear points
         *
         * @param vertices
        ...
        /**
         * From Eric Jordan's convex decomposition library Merges all parallel
         * edges in the list of vertices
         */
         ...
         *
         * @param vertices
         * @param distance The distance between points. Points closer than this
         * will be 'joined'
         */
